# US010 - See a pie chart representing, in percentage, the use of each piece of equipment

## 1. Requirements Engineering

### 1.1. User Story Description

As a GSM, I want to generate a pie chart representing, in percentage, the use of each piece of
equipment

### 1.2. Customer Specifications and Clarifications 

**From the specifications document:**

> Consider that the park has the following equipment: walking paths, children’s playground, picnic area, and exercise machines (gymnastics equipment).
  
> At the park exit there is an electronic device with a list of all
the equipment, in which the user(s) must indicate the equipment they
used that day

> In the file ”EquipmentUsed.csv” the choices of 1000 users are recorded.
Make a pie chart representing, in percentage, the use of each piece of
equipment. 

**From the client clarifications:**

> **Question:** 
> 
> **Answer:** 

> **Question:** 
>
> **Answer:**  

> **Question:** 
>
> **Answer:** 


### 1.4. Found out Dependencies

"At the park exit there is an electronic device with a list of all
the equipment, in which the user(s) must indicate the equipment they
used that day."

### 1.5 Input and Output Data

**Input Data:**
* CSV Data File
  * Contains Records:
    * the choices of 1000 users

**Output Data:**
* Pie chart
* Display confirmation success

### 1.7 Other Relevant Remarks

* Programming Language: Python
* Development environment: Jupyter Notebook
* Work delivery format: A single Jupyter Notebook file, which contains
all the work carried out
* Formulas must be written in LaTeX